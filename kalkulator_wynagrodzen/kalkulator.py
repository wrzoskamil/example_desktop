...
def odczytaj_pracownikow_z_pliku_csv(nazwa_pliku):
    lista_pracownikow = []
...
def oblicz_wyn_netto(lista_pracownikow):
    wynagrodzenia_netto = []
    for pracownik in lista_pracownikow:
        netto = pracownik.oblicz_wyn_netto()
        wynagrodzenia_netto.append((pracownik.imie, pracownik.nazwisko, netto))
    return wynagrodzenia_netto


pracownicy = odczytaj_pracownikow_z_pliku_csv(r'C:\Users\Kamil\example_desktop\kalkulator_wynagrodzen\pracownicy.csv')
wynagrodzenia_netto = oblicz_wyn_netto(pracownicy)
